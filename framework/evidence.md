...
the text the scanner flags. The exemption is exact at both levels: only a
finding from that rule whose matched text is covered by that declared text is
exempt; another rule or another offending text in the same block remains a
finding. This is a visible, reviewable claim, not a magic exemption: a made-up
role can make harmless prose appear to violate a rule, so review declarations
as carefully as the text they exempt.
